...
$query = "SELECT * FROM `problem` INNER JOIN `problem_version` WHERE `problem_version`.`vid`=`problem`.`default_version` ORDER BY `problem`.`pid` ASC LIMIT $start, $problem_per_page";
$result = mysql_query($query, $con);
?>
				...
				<div class="row">
					<ul class="nav nav-pills">
						<li role="presentation" class="<?php if($pages_len == 0 || $page == 1) echo 'disabled';?>">
							<a href="<?php if($pages_len != 0 && $pages_len != 0 && $page != 1) echo '/problem/list.php?page=' . ($page - 1); else echo 'javascript:void(0);';?>">上一页</a>
						</li>
						<?php if($pages_len <= 6): ?>
							<?php for($i = 1; $i <= $pages_len; $i++): ?>
								<li role="presentation" class="<?php if($page == $i) echo 'active'; ?>"><a href="/problem/list.php?page=<?php echo $i; ?>"><?php echo $i;?></a></li>
							<?php endfor; ?>
						<?php else: ?>
							<?php if($page > 2 && $page < $pages_len - 1): ?>
								...
								<li role="presentation" class="active"><a href="/problem/list.php?page=<?php echo $page; ?>"><?php echo $page; ?></a></li>
								<li role="presentation">
									<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
										...
									</a>
						...
						<li role="presentation" class="<?php if($pages_len == 0 || $page == $pages_len) echo 'disabled';?>">
							<a href="<?php if($pages_len != 0 && $page != $pages_len) echo '/problem/list.php?page=' . ($page + 1); else echo 'javascript:void(0);';?>">下一页</a>
						</li>
					</ul>
				</div>
			</div>
				...
				<tbody>
					<?php while($row = mysql_fetch_array($result)):?>
						<tr>
							...
							<td><a href="/problem/view.php?pid=<?php echo $row['pid'];?>"><?php echo $row['title']; ?></a></td>
							<td><?php echo $row['cnt_submission']; ?></td>
							<td><?php echo $row['cnt_submission_people']; ?></td>
							<td><?php echo $row['cnt_accepted_submission']; ?></td>
							<td><?php echo $row['cnt_accepted_people']; ?></td>
						</tr>
					<?php endwhile;?>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	$(".search-button").click(function() {
		window.location.href = "/problem/list.php?s=" + $("#search-input").val();
	});
	...
	$(".goto-button").click(function() {
		window.location.href = "/problem/list.php?page=" + $(".goto-input").val();
	});
</script>

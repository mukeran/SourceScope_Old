				...
				<div class="row">
					<ul class="nav nav-pills">
						<li role="presentation" class="<?php if($pages == 0 || $page == 1) echo 'disabled';?>">
							<a href="<?php if($pages != 0 && $pages != 0 && $page != 1) echo '/contest/list.php?page=' . ($page - 1); else echo 'javascript:void(0);';?>">上一页</a>
						</li>
						<?php if($pages <= 6): ?>
							<?php for($i = 1; $i <= $pages; $i++): ?>
								<li role="presentation" class="<?php if($page == $i) echo 'active'; ?>"><a href="/contest/list.php?page=<?php echo $i; ?>"><?php echo $i;?></a></li>
							<?php endfor; ?>
						<?php else: ?>
							<?php if($page > 2 && $page < $pages - 1): ?>
								...
								<li role="presentation" class="active"><a href="/contest/list.php?page=<?php echo $page; ?>"><?php echo $page; ?></a></li>
								<li role="presentation">
									<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
										...
									</a>
						...
						<li role="presentation" class="<?php if($pages == 0 || $page == $pages) echo 'disabled';?>">
							<a href="<?php if($pages != 0 && $page != $pages) echo '/contest/list.php?page=' . ($page + 1); else echo 'javascript:void(0);';?>">下一页</a>
						</li>
					</ul>
				</div>
			</div>
		...
		<div class="row" style="margin-top: 10px;">
			<table class="table table-striped table-hover table-bordered">
				...
				<tbody>
					...
					?>
					<tr class="<?php if($starttime_timestamp > time()) echo 'warning'; else if($endtime_timestamp > time()) echo 'success'; else echo 'info';?>">
						<td><?php echo $row['cid']; ?></td>
						<td><a href="/contest/view.php?cid=<?php echo $row['cid']; ?>"><?php echo $row['name']; ?></a></td>
						<td class="hidden-xs"><?php echo $row['contestform']; ?></td>
						<td><?php if($starttime_timestamp > time()) echo '未开始'; else if($endtime_timestamp > time()) echo '运行中'; else echo '已结束';?></td>
						<td class="hidden-xs"><?php echo $starttime; ?></td>
...
<script type="text/javascript">
	...
	$(".search-button").click(function() {
		window.location.href = "/contest/list.php?s=" + $("#search-input").val();
	});
	...
	$(".goto-button").click(function() {
		window.location.href = "/contest/list.php?page=" + $(".goto-input").val();
	});
</script>
